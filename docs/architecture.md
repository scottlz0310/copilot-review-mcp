...
New watches use `review-raven://watch/{id}` as the canonical scheme.

The legacy scheme `copilot-review://watch/{id}` is accepted as a **deprecated alias** for read and subscribe operations. This allows clients and databases that still carry the old URI (populated before the rename) to continue functioning without data migration.

- New URIs generated by `resourceURIForWatch()` use the canonical scheme only.
- `parseWatchIDFromURI()` and `SubscribeHandler` accept both schemes.
- Removal of the legacy alias is tracked separately. See [review-raven #65](https://github.com/scottlz0310/review-raven/issues/65) (to be filed).

### Environment variables

`REVIEW_RAVEN_GATEWAY_INTERNAL_URL` and `REVIEW_RAVEN_GATEWAY_INTERNAL_SECRET` are the canonical names. The legacy names `COPILOT_REVIEW_GATEWAY_INTERNAL_URL/SECRET` remain functional as fallbacks.
