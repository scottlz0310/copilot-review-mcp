...
1. `git status --porcelain` が空（untracked files を含む未コミット変更が一切ない状態）であること。ローカル作業ツリーの未コミット変更をレビュー根拠に使用してはならない。
...
2. 検証環境の `git status --porcelain` が空であり、変更がないこと
3. 現在の GitHub 上の PR HEAD SHA が `reviewedHeadSha` のままであること

次の場合は stale review として投稿（inline comment や APPROVE）を停止する。特に APPROVE 投稿直前に PR HEAD SHA を再取得し、レビュー開始時の SHA と異なる場合は APPROVE せず、最新 SHA に対してレビューをやり直す。
...
- candidate queue などの再レビュー依頼や、PRコメント（issue comment）の最新のアノテーション（またはコメント本文内の `- Expected PR HEAD:` 行）から `expected_head` が抽出できる場合、開始時に以下を照合する。
  ```text
...
- レビュー結果（ユーザー報告やサマリ）には必ず確認した `reviewedHeadSha` を明記して記録する。
