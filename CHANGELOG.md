...
- `docs/skills/thread-owl-review-cycle.md` / `.ja.md` — updated to address all unresolved review comments (both inline threads and review body actionable comments) regardless of the author. Updated to use `review-raven` MCP tools (`get_review_threads`, `reply_and_resolve_review_thread`, etc.) instead of direct `gh` CLI (GraphQL) for fetching, replying, and resolving threads. Added update instructions for installed skill templates. ([Issue #76](https://github.com/scottlz0310/review-raven/issues/76))
