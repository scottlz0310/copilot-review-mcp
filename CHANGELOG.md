...
- `pr-review-cycle` and `review-raven-thread-owl-cycle` skill templates now fail closed before reading PR comment bodies. Only comments from `scottlz0310-user`, Copilot, Thread Owl, and Codecov are trusted, including each GitHub App's GraphQL login without `[bot]` and REST login with `[bot]`; any other or unverifiable author stops automation for human escalation without quoting or acting on the comment body.
