...
- `docs/skills/thread-owl-review-cycle.md` / `.ja.md` — updated to address all unresolved review comments regardless of the author. Updated to use `review-raven` MCP tools (`get_review_threads`, `reply_and_resolve_review_thread`, etc.) instead of direct `gh` CLI (GraphQL) for fetching, replying, and resolving threads. Documented detailed procedures for fetching, replying to, tracking progress, and re-verifying non-thread comments like review bodies and PR comments (issue comments). Added update instructions for installed skill templates. ([Issue #76](https://github.com/scottlz0310/review-raven/issues/76))
