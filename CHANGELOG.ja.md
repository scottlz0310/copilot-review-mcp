...
- `docs/skills/thread-owl-review-cycle.md` / `.ja.md` — 投稿者を問わずすべての未解決レビュー指摘（インラインスレッドおよび review body 等の指摘）に対応するよう更新。スレッドの取得・返信・解決に `gh` CLI（GraphQL）ではなく `review-raven` MCP ツール（`get_review_threads`, `reply_and_resolve_review_thread`等）を使用するように更新。インストール済みSkillテンプレートへの更新手順を追記。([Issue #76](https://github.com/scottlz0310/review-raven/issues/76))
